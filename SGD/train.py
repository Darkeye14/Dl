...
from bench.utils import set_seed, accuracy


def parse_args():
    p = argparse.ArgumentParser(description="Train with SGD Momentum")
    ...
    p.add_argument("--weight-decay", type=float, default=5e-4, help="Weight decay (L2)")
    p.add_argument("--nesterov", action="store_true", help="Use Nesterov momentum")
...
def main():
    ...
    run_dir = os.path.join("runs", "sgd_momentum", args.dataset, f"seed_{args.seed}")
    ensure_dir(run_dir)
