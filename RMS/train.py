...
def parse_args():
    p = argparse.ArgumentParser(description="Train with RMSProp")
    ...
    p.add_argument("--alpha", type=float, default=0.99, help="RMSProp alpha (EMA decay)")
    p.add_argument("--eps", type=float, default=1e-8, help="RMSProp epsilon")
    p.add_argument("--momentum", type=float, default=0.0, help="RMSProp momentum")
    p.add_argument("--centered", action="store_true", help="Use centered RMSProp")
...
def main():
    ...
    run_dir = os.path.join("runs", "rmsprop", args.dataset, f"seed_{args.seed}")
    ensure_dir(run_dir)
