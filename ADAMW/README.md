...
A minimal, reproducible project to train and evaluate AdamW on MNIST/CIFAR-10. Includes a runnable training harness, CSV logs, and a simple reporting script.

## Quickstart (Windows)

- Prereqs: Python 3.9–3.12
- Open a terminal in the project folder `ADAMW`.
...
python report.py --csv runs/adamw/mnist/seed_42/metrics.csv --plot
...
- CSV logs per run at:
```
runs/adamw/<dataset>/seed_<N>/metrics.csv
```
...
```
python report.py --csv runs/adamw/<dataset>/seed_<N>/metrics.csv --plot
```
Outputs:
...
## Notes
- AdamW uses decoupled weight decay (better-behaved than Adam with L2), defaults: betas=(0.9, 0.999), eps=1e-8.
